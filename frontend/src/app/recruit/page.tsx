...
export default function RecruitListPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        {isAuthenticated && (
          <button
            ...
            className={`inline-flex items-center gap-1.5 rounded-xl border px-4 py-1.5 text-sm font-medium transition-all ${
              appliedSameSchool
      ...
      <div className="flex gap-2">
        <div className="flex flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm transition-all focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100">
          ...
          <input
            ...
            placeholder="이름, 학과, 직군, 기술 스택으로 검색"
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
          {appliedKeyword && (
            <span className="flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1 text-slate-600">
              ...
              {appliedKeyword}
              <button onClick={() => { setAppliedKeyword(''); setKeyword(''); }} className="text-slate-300 hover:text-slate-600">
          ...
          {appliedSameSchool && (
            <span className="flex items-center gap-1.5 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-indigo-700">
              🏫 같은 학교
              <button onClick={() => setAppliedSameSchool(false)} className="text-indigo-300 hover:text-indigo-600">
                <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
          ...
          {appliedRoles.map((role) => (
            <span key={role} className="flex items-center gap-1.5 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-indigo-700">
              {role}
              <button onClick={() => setAppliedRoles(appliedRoles.filter((r) => r !== role))} className="text-indigo-300 hover:text-indigo-600">
                <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
          ...
          {appliedSkills.map((skill) => (
            <span key={skill} className="flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-slate-600">
              {skill}
              <button onClick={() => setAppliedSkills(appliedSkills.filter((s) => s !== skill))} className="text-slate-300 hover:text-slate-600">
                <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
          ...
          <button
            onClick={() => { setAppliedKeyword(''); setKeyword(''); setAppliedRoles([]); setAppliedSkills([]); setAppliedSameSchool(false); }}
                  ...
                  <div className="flex translate-y-[2px] gap-2">
                    {isAuthenticated && (
                      <button
                        ...
                        className="rounded-xl border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-600 transition-all hover:border-slate-700 hover:bg-slate-700 hover:text-white"
                      >
                        스카우트
                      </button>
                    )}
      ...
      {/* 검색 필터 모달 */}
      {isFilterOpen && (
        <div onClick={() => setIsFilterOpen(false)} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-[500px] rounded-3xl bg-white p-7 shadow-2xl">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-900">검색 필터</h2>
              <button onClick={() => setIsFilterOpen(false)} className="rounded-full p-1 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600">
            ...
            <p className="mb-2 text-sm font-semibold text-slate-700">직군</p>
